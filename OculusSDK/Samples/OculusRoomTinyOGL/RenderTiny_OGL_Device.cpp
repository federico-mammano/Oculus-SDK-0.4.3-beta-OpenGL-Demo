...
Filename    :   RenderTiny_OGL_Device.cpp
...
RenderDevice::RenderDevice(const RendererParams& p, HWND win, HGLRC gl)
    : Window(win)
    , WglContext(gl)
...
void RenderDevice::InitRenderDevice(const RendererParams& p)
{
    Params = p;

	// Initialize render params.
    memset(&RParams, 0, sizeof(RParams));
	RParams.Window		= Window;
	RParams.DC			= GetDC(Window);
	RParams.RTSize.w	= Params.Resolution.w;
	RParams.RTSize.h	= Params.Resolution.h;
	RParams.Multisample	= Params.Multisample;
...
void RenderDevice::InitShaders( const char * vertex_shader, const char * pixel_shader, ShaderSet ** pShaders)
{
	(*pShaders) = CreateShaderSet();
	(*pShaders)->SetShader(new Shader(this, Shader_Vertex, vertex_shader));
	(*pShaders)->SetShader(new Shader(this, Shader_Fragment, pixel_shader));
}
...
void RenderDevice::SetRenderTarget(Texture* color, Texture* depth, Texture* stencil)
{
    OVR_UNUSED(stencil);
...
void RenderDevice::SetTexture(ShaderStage, int slot, const Texture* t)
{
...
Fill* RenderDevice::CreateTextureFill(Texture* t, bool useAlpha)
{
...
void RenderDevice::Render(const ShaderFill* fill, Buffer* vertices, Buffer* indices)
{
    ...
    Render(fill ? (const Fill*)fill : (const Fill*)DefaultFill,
           vertices, indices,
           matrix, 0, (int)(indices->GetSize()/sizeof(unsigned short)), Prim_Triangles, Mesh_Distortion_OGL);
}

void RenderDevice::Render(const Fill* fill, Buffer* vertices, Buffer* indices,
                      const Matrix4f& matrix, int offset, int count, PrimitiveType rprim, MeshType meshType /*= Mesh_Scene*/)
{
    ...
    case Mesh_Distortion_OGL:
		glVertexAttribPointer(0, 2, GL_FLOAT, false, sizeof(ovrDistortionVertex), reinterpret_cast<char*>(offset) + offsetof(ovrDistortionVertex, ScreenPosNDC));
        glVertexAttribPointer(1, 2, GL_FLOAT, false, sizeof(ovrDistortionVertex), reinterpret_cast<char*>(offset) + offsetof(ovrDistortionVertex, TimeWarpFactor));
        glVertexAttribPointer(2, 2, GL_FLOAT, false, sizeof(ovrDistortionVertex), reinterpret_cast<char*>(offset) + offsetof(ovrDistortionVertex, TanEyeAnglesR));
        glVertexAttribPointer(3, 2, GL_FLOAT, false, sizeof(ovrDistortionVertex), reinterpret_cast<char*>(offset) + offsetof(ovrDistortionVertex, TanEyeAnglesG));
        glVertexAttribPointer(4, 2, GL_FLOAT, false, sizeof(ovrDistortionVertex), reinterpret_cast<char*>(offset) + offsetof(ovrDistortionVertex, TanEyeAnglesB));
...
void RenderDevice::RenderWithAlpha(const Fill* fill, Buffer* vertices, Buffer* indices,
